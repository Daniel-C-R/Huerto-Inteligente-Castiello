// Código adaptado para funcionar con BMP280 en vez de BME280.
// No lee humedad ambiente.
...
// Definir los pines I2C para el BMP280
#define I2C_SCL 20
#define I2C_SDA 21
...
void connectMQTT() {
  // Hay que poner un identificador de conexión único a cada micocontrolador
...
void setup() {
  ...
  if (!bmp.begin(0x76)) {
    Serial.println("No se pudo encontrar un sensor BMP280 válido, ¡verifica el cableado!");
    // Indicamos en magenta que el BMP280 no está bien conectado
...
void loop() {
  ...
  if (now - lastMsg > 5000) {
    ...
    float pressure = bmp.readPressure() / 100.0F;  // Conversión a hPa
    float altitude = bmp.readAltitude(1013.25);    // Ajusta según la presión local
